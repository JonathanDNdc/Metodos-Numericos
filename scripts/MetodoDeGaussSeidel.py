...
def gauss_seidel(funciones_str, initials_str, err=0.000000001, it=250):
    ...
    while not all(n < err for n in errs):
        ...
        for func, var, var1 in zip(mat, variables, range(len(variables))):
            ...
            for var2, var3 in zip(variables, range(len(variables))):
                if var != var2:
                    a -= new_initials[str(var2)] * func[var3]
            try:
                a /= func[var1]
            except ZeroDivisionError:
                return "Reordena la matriz", initials, mat, funciones
        ...
        if itn > it:
            return "No converge", initials, mat, funciones

    for key, value in new_initials.items():
        new_initials[key] = round(value, 6)

    return new_initials, initials, mat, funciones
